<template lang="jade">
	.calendar
		.input-wrapper(v-show='showInput')
			.input(v-text='value',:style='{width: inputWidth}' style='text-indent:30px;' @click='togglePanel = !togglePanel')
			span.input-clear(@click='clearValue')
		transition(name='toggle')
			.pannel-wrapper(:style='themePannelBg' v-show='togglePanel')
				.pannel-header(:style='themeHeaderBg')
					span.year(:style='themeHeaderYear' v-text='tmpYear' @click='showYearPannel')
					span(:style='themeHeaderSep') /
					span.month(:style='themeHeaderMonth' v-text='tmpMonth + 1' @click='showMonthPannel')
					span.prev(@click='prevMonth',:style='themeLeftArrow' v-show='pannelType !== "month"') &lt;
					span.next(@click='nextMonth',:style='themeRightArrow' v-show='pannelType !== "month"') &gt;
				.year-list(v-show='pannelType === "year"')
					ul.month-wrapper
						li(v-for='item in yearList',:class="{selected: isSelected(item, 'year')}" @click='selectYear(item)') {{item}}
				.month-list(v-show='pannelType === "month"')
					ul.month-wrapper
						li(v-for='item in monthList',:class="{selected: isSelected(item, 'month')}" @click='selectMonth(item)') {{month(item, lang)}}
				.date-list(v-show='pannelType === "date"')
					ul.week
						li(v-for='item in weekList' ,:style='themeWeekColor') {{week(item, lang)}}
					ul.date
						li(v-for='item in dateList' ,:class="{selected: isSelected(item, 'date'), 'notCurMonth': !item.isCurMonth, unvalid: !validDate(item)}" @click='selectDate(item)' ,:style='setSeltheme(item, "date")') {{item.value}}
					.time(v-show='type === "time"')
						input(type='text' v-model='startHour' @focus='checkTime("startHour")' @blur='clearCheck')
						| : 
						input(type='text' v-model='startMin'  @focus='checkTime("startMin")' @blur='clearCheck')
						| - 
						input(type='text' v-model='endHour'  @focus='checkTime("endHour")' @blur='clearCheck')
						| : 
						input(type='text' v-model='endMin'  @focus='checkTime("endMin")' @blur='clearCheck')
				.group-btn(v-show='pannelType != "date"')
					button.btn.btn-confirm(type='button' @click='confirmSelect',:style='themeBtnCon') 确认
					button.btn.btn-cancle(type='button' @click='cancleSelect',:style='themeBtnCan') 取消
		.mask(v-show='togglePanel' style='z-index:1;background-color:rgba(255,255,255,0)' @click='togglePanel=false')
</template>
...
export default {
				...
				methods: {
								...
								switch(this.type) {
										case 'single':
												if((this.startMonth + 1)<10){
													this.startMonth='0'+(this.startMonth + 1)
												}else{
												...
												this.value = `${this.startYear}${this.format}${this.startMonth}${this.format}${this.startDate}`;
												break;
										case 'range':
						...
						setSeltheme:function(item, type) {
						...
						getValue() {
								...
								// if(localStorage.ddys_medicalDate){
								// 	this.$emit('getValue','2017-09-28')
								// }else{
								// 	this.$emit('getValue', this.value);
								// }
						},
						cancle(){
							this.$emit('cancle', this.cancleSelect);
						},
						getStartTime() {
